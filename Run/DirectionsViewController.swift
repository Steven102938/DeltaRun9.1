...
//        println(distancetostepOne)
    ...
    func calculateDisplayDirections () {
        ...
        for step in global.steps {
         ...
         routeInstructions = step["html_instructions"] as! String
            let routeLatitude = (step["start_location"] as! Dictionary<NSObject, AnyObject>)["lat"] as! Double
            let routeLongitude = (step["start_location"] as! Dictionary<NSObject, AnyObject>)["lng"] as! Double
            
            let endCoordinate = CLLocationCoordinate2DMake(CLLocationDegrees(routeLatitude), CLLocationDegrees(routeLongitude))
    ...
    func displayDirections () {
        ...
        var camera: GMSCameraPosition = GMSCameraPosition.cameraWithTarget(global.routeFocusCoordinates[global.directionsNumber - 1], zoom: 17, bearing: 30, viewingAngle: 40)
        DirectionsMapView.camera = camera

        
        var DirectionsTextSwift = try? NSAttributedString(

...
class MapOverviewViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate {
    ...
    struct global {
    static var startCoordinates = [CLLocation]()
    }
    ...
    struct loc {
        ...
        static var randomRotate: Double = Double(arc4random_uniform(360))
        ...
        print("\(loc.randomRotate)")
            ...
            print("dispatching")
                    ...
                    if (error != nil) {
                        ...
                        var startCoordinatesLat:Double?
                        var startCoordinatesLong:Double?
                        var startCllocation:CLLocation?
                            ...
                            for leg in legs {
                                ...
                                steps = leg["steps"] as! Array<Dictionary<NSObject,  AnyObject>>
                            }
                        for step in steps {
                            startCoordinatesLat = (step["start_location"] as! Dictionary<NSObject, AnyObject>)["lat"] as? Double
                            startCoordinatesLong = (step["start_location"] as! Dictionary<NSObject, AnyObject>)["lng"] as? Double
                            startCllocation = CLLocation(latitude: startCoordinatesLat!, longitude: startCoordinatesLong!)
                            global.startCoordinates.append(startCllocation!)
                        }
...
//    }
    ...
    func locationManager(manager: CLLocationManager, didUpdateLocations locations: [CLLocation]){
        ...
        print("\(sinOfDistanceOne)" + "\(cosOfDistanceOne)")
        ...
        print("locations = \(loc.locValueOne.latitude) \(loc.locValueOne.longitude)")
        print("\(loc.CalculateRouteDistance)")
        ...
        print("locations = \(loc.locValueTwo.latitude) \(loc.locValueTwo.longitude)")
        print("\(loc.CalculateRouteDistance)")
        ...
        print("locations = \(loc.locValueThree.latitude) \(loc.locValueThree.longitude)")
        print("\(loc.CalculateRouteDistance)")
